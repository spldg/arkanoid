...
import { BRICK_HEIGHT, BRICK_WIDTH, firstLevel, GAME_WIDTH, PIECE_SIZE, startY } from '../constants'
...
export class BrickSystem extends PIXI.Container {
    ...
    private draw(): void {
        ...
        for (let row = 0; row < firstLevel.length; row++) {
            for (let col = 0; col < firstLevel[row].length; col++) {
                ...
                const brick = new Brick(this.texture!)
    ...
    private playDestroyAnimation(brick: Brick) {
        const pieceArr = []
        for (let i = 0; i <= 14; i++) {
